...
const { JWT_URL_1, JWT_SECRET_1 } = require('../../config')

module.exports = async data => {
  logger('info', ['post-data', data._id, 'jobId', data.jobId, 'system', data.system, 'start'])
  if (config.JWT_URL_1 && data.content.url.includes(JWT_URL_1)) {
    axios.defaults.headers.common['Authorization'] = generateSystemJwt(JWT_SECRET_1)
  } else {
    axios.defaults.headers.common['Authorization'] = generateSystemJwt(JWT_SECRET)
  }
